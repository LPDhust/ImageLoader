...
import com.example.lpd.imageloadertest.utils.MyUtils;
...
public class ImageLoader {
    ...
    public void bindBitmap(final String uri, final ImageView imageView, final int imageWidth, final int imageHeight) {
        ...
        Runnable loadBitmapTask = new Runnable() {
            @Override
            public void run() {
                Bitmap bitmap = loadBitmap(uri, imageWidth, imageHeight);
                if(bitmap != null){
    ...
    private Bitmap loadBitmap(String uri, int imageWidth, int imageHeight) {
        Bitmap bitmap = loadBitmapFromMemCache(uri);
        if(bitmap != null){
        ...
        try{
            bitmap = loadBitmapFromDiskCache(uri, imageWidth, imageHeight);
            if(bitmap != null){
                Log.d(TAG, "loadBitmapFromDiskCache,uri:" + uri);
                return bitmap;
            }
            bitmap = loadBitmapFromHttp(uri, imageWidth, imageHeight);
    ...
    private Bitmap loadBitmapFromHttp(String uri, int imageWidth, int imageHeight) throws IOException {
        ...
        return loadBitmapFromDiskCache(uri, imageWidth, imageHeight);
    }

    public boolean downloadUriToStream(String uri, OutputStream outputStream){
        HttpURLConnection urlConnection = null;
    ...
    private Bitmap loadBitmapFromDiskCache(String uri, int imageWidth, int imageHeight) throws IOException {
        if(Looper.myLooper() == Looper.getMainLooper()){
        ...
        if(snapShot != null){
            ...
            bitmap = mImageResizer.decodeSampleBitmapFromFileDescriptor(fileDescriptor, imageWidth, imageHeight);
            if (bitmap != null){
    ...
    private Bitmap loadBitmapFromMemCache(String uri) {
        final String key = hashKeyForUri(uri);
        Bitmap bitmap = getBitmapFromMemCache(key);
        return bitmap;
    }

    private String hashKeyForUri(String uri) {
        String cacheKey;
        try{
